package com.euler.solutions.problem05to10;

/**
 * @author sukrityv
 *
 */
public class Problem08_LargestProductInSeries {

	public static void main(String[] args) {

		String inputStr = "";
		int seriesLength = 13;

		long startTime = System.currentTimeMillis();

		System.out.println(getSum(inputStr, seriesLength));

		long endTime = System.currentTimeMillis();
		System.out.println("That took " + (endTime - startTime) + " milliseconds");
	}
	
	static long getSum(String inputStr, int size) {

		long result = 0;
		int index = 1;

		String[] array = inputStr.split("");

		int temp = 0;
		for (int i = 1; i <= size; i++) {
			temp += Integer.parseInt(array[i]);
		}
		result = temp;

		int counter = 0;
		for (int i = 1; i + size < array.length; i++) {
			int newVal = Integer.parseInt(array[i + size]);

			if (newVal == 0) {
				counter = size+1;
			}

			temp = temp + newVal - Integer.parseInt(array[i]);

			if (result < temp && counter==0) {
				result = temp;
				index = i + 1;
			}
			counter = counter > 0 ? --counter : 0;
		}
		System.out.println(index);

		result = 1;
		for (int i = index; i < index + size; i++) {
			System.out.print(array[i]);
			result *= Integer.parseInt(array[i]);
		}
		System.out.println();

		return result;
	}
	
}
